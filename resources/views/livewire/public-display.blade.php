...
<div class="min-h-screen bg-slate-100 flex flex-col font-sans select-none" wire:poll.2s="checkQueueUpdate">
    ...
    <header class="bg-slate-900 text-white px-6 py-4 shadow-md flex justify-between items-center border-b-4 border-orange-500">
        <div class="flex items-center gap-4">
            <img src="{{ asset('logo kota makassar.png') }}" alt="Logo Kota Makassar" class="h-14 w-auto object-contain drop-shadow-sm shrink-0" />
            <img src="{{ asset('logo kecamatan panakkukang.jpg') }}" alt="Logo Kecamatan Panakkukang" class="h-14 w-auto object-contain rounded-xl shadow-xs shrink-0" />
        ...
        <aside class="w-full lg:w-[440px] flex flex-col gap-4" aria-label="Panggilan antrean">
            <div class="flex-1 flex flex-col gap-4">
                @forelse($this->serviceStatuses as $serviceStatus)
                    ...
                    <section id="queue-call-{{ strtolower($serviceStatus['code']) }}" class="flex-1 bg-white rounded-2xl shadow-md border-t-8 {{ $isKtp ? 'border-orange-600' : 'border-emerald-600' }} border-x border-b border-slate-200 px-5 py-5 text-center transition-all duration-300 flex flex-col items-center justify-center" aria-labelledby="queue-call-{{ strtolower($serviceStatus['code']) }}-heading">
                        ...
                        @if($serviceStatus['count'] > 1)
                            <div class="grid {{ $serviceStatus['count'] === 2 ? 'grid-cols-2' : 'grid-cols-3' }} gap-2 mt-4">
                                @foreach($serviceStatus['numbers'] as $number)
                                    <div class="rounded-xl border {{ $isKtp ? 'border-orange-200 bg-orange-50' : 'border-emerald-200 bg-emerald-50' }} px-2 py-2.5">
                                        <span class="block text-[10px] font-bold uppercase tracking-widest {{ $isKtp ? 'text-orange-600' : 'text-emerald-600' }}">Nomor</span>
                                        <span class="block mt-1 text-2xl font-black font-mono {{ $isKtp ? 'text-orange-950' : 'text-emerald-950' }}">{{ $number }}</span>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <p class="mt-4 text-8xl font-black font-mono tracking-tight {{ $isKtp ? 'text-orange-950' : 'text-emerald-950' }}">{{ $serviceStatus['display_number'] }}</p>
                        @endif

                        <div class="mt-4 pt-3 border-t {{ $isKtp ? 'border-orange-100' : 'border-emerald-100' }} w-full">
                            <p class="text-xs font-semibold {{ $isActive ? ($isKtp ? 'text-orange-700' : 'text-emerald-700') : 'text-slate-500' }}">{{ $isActive ? 'Sedang Dilayani di Ruang Pelayanan' : $serviceStatus['status_label'] }}</p>
                        </div>
                    </section>
                @empty
